    ...
    return (
      <div className="flex justify-center items-center min-h-64">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-light-600 mx-auto mb-4"></div>
          ...
          {menuItems.map((item) => (
            <Link
              ...
              className={`group relative overflow-hidden rounded-3xl border transition-all duration-500 hover:shadow-2xl hover:scale-105 transform-gpu ${
                isActive(item.path)
                  ? "border-blue-light-400 shadow-2xl scale-105 ring-4 ring-blue-light-500/20"
                  : "border-gray-200/60 dark:border-gray-700/60 hover:border-blue-light-300/50 dark:hover:border-blue-light-600/50"
              ...
              <div className="relative p-8 z-10 h-full flex flex-col">
                ...
                <div className="flex items-center justify-center mb-6 flex-1">
                  <div className={`relative w-20 h-20 rounded-2xl flex items-center justify-center transform transition-all duration-500 group-hover:scale-110 ${
                    isActive(item.path)
                      ? "bg-white/80 dark:bg-gray-800/80 shadow-xl ring-2 ring-blue-light-500/30"
                      : "bg-white/60 dark:bg-gray-800/60 shadow-lg group-hover:shadow-xl group-hover:ring-2 group-hover:ring-blue-light-400/20"
                  } backdrop-blur-sm`}>
                    {item.logo ? (
                      ...
                      <div className={`text-3xl transition-all duration-500 ${
                        isActive(item.path)
                          ...
                          : "text-gray-600 dark:text-gray-400 group-hover:text-blue-light-500 dark:group-hover:text-blue-light-400"
                      }`}>
                        {item.icon}
                      </div>
                    )}
                ...
                <div className="text-center flex flex-col items-center">
                  <h3 className={`text-xl font-bold mb-3 transition-colors duration-300 ${
                    isActive(item.path)
                      ? "text-blue-light-900 dark:text-blue-light-100"
                  ...
                  {item.description && (
                    <p className={`text-sm leading-relaxed transition-colors duration-300 ${
                      isActive(item.path)
                        ? "text-blue-light-700/90 dark:text-blue-light-300/90"
                ...
                {isActive(item.path) && (
                  <div className="absolute top-4 right-4">
                    <div className="relative">
                      <div className="w-4 h-4 bg-blue-light-500 rounded-full animate-ping absolute"></div>
                      <div className="w-4 h-4 bg-blue-light-600 rounded-full relative"></div>
                    </div>
                  </div>
                )}
                ...
                <div className={`absolute bottom-4 right-4 transform transition-all duration-300 ${
                  ...
                  <div className="w-6 h-6 bg-blue-light-500 rounded-full flex items-center justify-center">
